...
class MLPTrainer:
    ...
    def train_step(self, images, _targets=None):
        ...
        with torch.no_grad():  # 만약 YOLO 내부 일부가 학습 모드라면 batchnorm 등이 바뀔 수 있으므로, eval로 고정했음
            raw_preds = self.yolo.model(enhanced)  # list[Tensor] or Tensor depending on version

        # YOLOv8 버전에 따라 raw_preds가 Tensor 하나로 올 수도 있고, 여러 스케일을 리스트로 반환할 수도 있다.
        ...
        conf_logits_list = []
        for p in preds_list:
            # p.shape == [B, n_anchors, grid_h, grid_w, 5 + nc] (ex: 80 classes)
            # p[..., 4] 가 objectness(logit) 부분
            # → shape: [B, n_anchors, grid_h, grid_w]
            conf_logits = p[..., 4]
            # flatten 뒤 [B, n_anchors * grid_h * grid_w]
            conf_logits_flat = conf_logits.flatten(1)
            conf_logits_list.append(conf_logits_flat)
