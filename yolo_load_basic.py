...
# 이미지 읽기
img = cv2.imread('download.jpeg')  # 입력하고 싶은 이미지 경로
...
if rois:
    ...
    # ROI 외부 영역 만들기
    mask = np.zeros(img.shape[:2], dtype=np.uint8)  # 검은색 마스크 생성
    cv2.rectangle(mask, (large_roi[0], large_roi[1]), (large_roi[2], large_roi[3]), 255, thickness=-1)  # ROI 영역 흰색 칠하기

    # ROI 바깥 영역은 mask가 0인 부분
    outside_img = img.copy()
    outside_img[mask == 255] = (0, 0, 0)  # ROI 영역을 검은색으로 처리

    # 큰 바운딩 박스를 이미지에 그리기
    cv2.rectangle(img, (large_roi[0], large_roi[1]), (large_roi[2], large_roi[3]), (0, 255, 0), 2)

# 결과 이미지 저장
cv2.imwrite('result_with_large_roi.jpg', img)
cv2.imwrite('result_large_roi.jpg', roi_img)
cv2.imwrite('result_without_large_roi.jpg', outside_img)
