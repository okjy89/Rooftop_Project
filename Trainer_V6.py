...
from MLPNetworkV5 import DeepKernelPredictor  # 기존에 정의한 클래스
...
class MLPTrainer:
    def __init__(self, backbone, feature_channels,
        ...
        # 4) Optimizer:kernel predictor만 업데이트
        self.optimizer = optim.Adam(self.kernel_pred.parameters(), lr=lr) #V%에서 Yolov5 backbone 추출로 변경되며 backbone 학습 안 시킴킴
    ...
    def train_step(self, images, _targets=None, epoch=0, batch_idx=0):
        ...
        '''
        # (NEW) Gradient 흐름 확인 (1회만 출력해도 됨)
        if batch_idx % 10 == 0:
            print("\n[Gradient Check] kernel_pred parameter gradients:")
            for name, param in self.kernel_pred.named_parameters():
                if param.grad is not None:
                    print(f" - {name}: grad mean = {param.grad.abs().mean():.4e}, max = {param.grad.abs().max():.4e}")
                else:
        ...
        # 첫 번째 원본 이미지 저장
        raw_img = imgs[0].detach().cpu()  # [3, H, W]
        save_image(raw_img, f"{raw_dir}/raw_epoch{epoch}_batch{batch_idx}.png")

        # 첫 번째 필터링 이미지 저장
        filtered_img = enhanced[0].detach().cpu()  # [3, H, W]
    ...
    def fit(self, dataloader, epochs=10):
        for epoch in range(1, epochs + 1):
            ...
            # ---------------------------------------------
            # 배치별로 진행 상황을 실시간으로 출력하기 위해
            # dataloader를 enumerate로 돌면서 batch_idx를 가져옵니다.
            # ---------------------------------------------
            for batch_idx, (imgs, _) in enumerate(dataloader, start=1):
                ...
                # 같은 줄을 덮어쓰기 위해 '\r'을 맨 앞에 두고, end=""로 자동 줄바꿈을 막습니다.
                print(
            ...
            # 한 에포크가 끝난 뒤에는 줄바꿈을 하고 평균 값을 출력합니다.
            avg_loss = total_loss / num_batches
            avg_conf = total_conf / num_batches
            print(  # '\n' 없이도 자동 줄바꿈되지만, 보기 좋게 구분하기 위해 개행 없이 사용
                f"\n>> Ep {epoch}/{epochs} 완료 — "
